...
const int rs = 6, en = 7, d4 = 9, d5 = 10, d6 = 11, d7 = 12; // digital pins for screen
...
void setup() {
  ...
  // Read the roundieCount from EEPROM
  EEPROM.get(address, roundieCount);

  // Initialize the LCD and display the roundieCount
  lcd.setCursor(0, 0);
  lcd.print("Roundie Count:");
  lcd.setCursor(0, 1);
  lcd.print(roundieCount);
}
...
void loop() {
  ...
  if (buttonState != lastButtonState) {
    if (buttonState == HIGH) {
      // Only update the LCD when the button is pressed
      isLcdOn = true;
      ...
      roundieCount++;
      EEPROM.put(address, roundieCount);
    }
  }
